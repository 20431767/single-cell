    ...
    foreach ($lambda_c in $lambda_cs) {
        foreach($lambda_d in $lambda_ds) {
            foreach($lambda_e in $lambda_es) {
                $output_dir="test1-entropy-mask-lambda_b_$lambda_b-lambda_c_$lambda_c-lambda_d_$lambda_d-lambda_e_$lambda_e"
                foreach ($dim in $dims) {
                    conda activate cgi2
                    foreach ($method in $methods) {
                        foreach ($dataset_dir in $dataset_dirs) {
                            ...
                            foreach($i in 1..$repeat_num) {
                                python run.py --dim $dim --data-dir $data_dir --bench-dataset $dataset_dir --output-dir $output_dir --output-ari-file "$exp_name.ari.txt" --seed $i --method $method --lambda_b $lambda_b --lambda_c $lambda_c --lambda_d $lambda_d --lambda_e $lambda_e  --epochs $epochs --generate-files
                                python run.py --dim $dim --data-dir $data_dir --bench-dataset $dataset_dir --output-dir $output_dir --output-ari-file "$exp_name.ari.txt" --seed $i --method $method --L2-normalization --lambda_b $lambda_b --lambda_c $lambda_c --lambda_d $lambda_d --lambda_e $lambda_e  --epochs $epochs
                                ...
                                python run.py --dim $dim --inherit-centroids --data-dir $data_dir --bench-dataset $dataset_dir --output-dir $output_dir --output-ari-file "$exp_name.ari.txt" --seed $i --method $method --L2-normalization --lambda_b $lambda_b --lambda_c $lambda_c --lambda_d $lambda_d --lambda_e $lambda_e   --epochs $epochs
                                # only one should
